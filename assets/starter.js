...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const applyAllFilters = () => {
            // let a = document.getElementById('job-listings-container');
            // a.style.display = 'none';
            searchInput.addEventListener("keyup", function(event){
            let user_input_data = event.target.value.toLowerCase();
             let sug = [];
            for(let i = 0; i < allJobs.length; i++){
                if(user_input_data == allJobs[i].company.toLowerCase()){
                    sug.push(allJobs[i]);
                } else if(user_input_data == allJobs[i].role.toLowerCase()){
                    sug.push(allJobs[i]);
                } else if(allJobs[i].position.toLowerCase().includes(user_input_data) == true){
                    sug.push(allJobs[i]);
                } else if(allJobs[i].description.toLowerCase().includes(user_input_data) == true){
                    sug.push(allJobs[i]);
                } else if(user_input_data == allJobs[i].level.toLowerCase()){
                    sug.push(allJobs[i]);
                };
                for(let j = 0; j < allJobs[i].skills.length; j++){
                    if(user_input_data == allJobs[i].skills[j].toLowerCase()){
                        sug.push(allJobs[i]);
                };
                };
    ...
    const initializeApp = async () => {
        ...
        //     let suggestions = [];
        //     var user_enter = document.getElementById("search-input").value;
